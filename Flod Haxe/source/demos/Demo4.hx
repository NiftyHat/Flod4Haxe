...
package demos ;
  ...
  import neoart.flod.fred.*;
   ...
   final class Demo4 extends Sprite {
    //[Embed(source="filename.mod", mimeType="application/octet-stream")]

    var   Song   : Class<Dynamic>;
    var  player : FEPlayer;

    public function new() {
      super();
      player = new FEPlayer();
      player.load(ASCompat.dynamicAs(Type.createInstance(Song, []) , ByteArray));
      if (player.version != 0) player.play();
    }
  }
