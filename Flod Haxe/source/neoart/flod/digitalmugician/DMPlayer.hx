  ...
  import neoart.flod.core.*;
   ...
   final class DMPlayer extends AmigaPlayer {
    ...
    override public function process() {
      var chan:AmigaChannel, dst:Int, i:Int, idx:Int, j:Int, len:Int, memory= amiga.memory, r:Int, row:AmigaRow, src1:Int, src2:Int, sample:DMSample, value:Int, voice:DMVoice;

      i = 0;while (i < numChannels) {
        voice = voices[i];
        sample = voice.sample;

        if (i < 3 || numChannels == 4) {
          chan = voice.channel;
          if (stepEnd != 0) voice.step = song1.tracks[trackPos + i];
          ...
          chan = mixChannel;
          if (stepEnd != 0) voice.step = song2.tracks[trackPos + (i - 3)];
        }
        ...
        if (patternEnd != 0) {
          ...
          if (row.note != 0) {
            if (row.effect != 74) {
              voice.note = row.note;
              if (row.sample != 0) sample = voice.sample = samples[row.sample];
            }
...
++i;
      ...
      if (version == DIGITALMUG_V2) {
        ...
        len = buffer1 + 700;
        for (_tmp_ in buffer1...len) {i = _tmp_;
amiga.memory[i] = 0;
};
      }
    }
...
i = _tmp_;
          ...
          sample.pointer += data;
          if (sample.loopPtr == 0) sample.loopPtr = sample.pointer + sample.loop;
        }
      } else {
...
j = _tmp_;
    ...
    public static inline final DIGITALMUG_V1 :ASAny= 1;
    public static inline final DIGITALMUG_V2 :ASAny= 2;
