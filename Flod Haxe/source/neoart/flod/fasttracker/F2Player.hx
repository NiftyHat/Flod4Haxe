    ...
    override public function process() {
      ...
      if (tick == 0) {
        ...
        while (voice != null) {
          ...
          com = row.volume >> 4;
          porta = ASCompat.toInt(row.effect == FX_TONE_PORTAMENTO || row.effect == FX_TONE_PORTA_VOLUME_SLIDE || com == VX_TONE_PORTAMENTO);
          paramx = row.param >> 4;
          voice.keyoff = 0;
          ...
          if (row.effect != 0) {
            ...
            switch (row.effect) {
              case FX_PORTAMENTO_UP:
                if (row.param != 0) voice.portaU = row.param << 2;
                
              case FX_PORTAMENTO_DOWN:
                if (row.param != 0) voice.portaD = row.param << 2;
                
              case FX_TONE_PORTAMENTO:
                if (row.param != 0 && com != VX_TONE_PORTAMENTO) voice.portaSpeed = row.param;
              ...
              case FX_SET_ENVELOPE_POSITION:
                if (voice.instrument == null || voice.instrument.volEnabled == 0) break;
                instr  = voice.instrument;
                value  = row.param;
...
{i = _tmp_;
              ...
              case FX_MULTI_RETRIG_NOTE:
                ...
                if (row.volume == 0 && voice.retrigy != 0) {
                  com = tick + 1;
                  if (com % voice.retrigy != 0) break;
                  if (row.volume > 80 && voice.retrigx != 0) retrig(voice);
                }
              ...
              case FX_EXTRA_FINE_PORTAMENTO:
                if (paramx == 1) {
                  if (paramy != 0) voice.xtraPortaU = paramy;
                  voice.period -= voice.xtraPortaU;
                  voice.flags |= UPDATE_PERIOD;
                } else if (paramx == 2) {
                  if (paramy != 0) voice.xtraPortaD = paramy;
        ...
        while (voice != null) {
          ...
          if (row.volume != 0) {
            ...
            switch (paramx) {
              ...
              case VX_TONE_PORTAMENTO:
                if (voice.portaPeriod != 0) voice.tonePortamento();
          ...
          switch (row.effect) {
            ...
            case FX_MULTI_RETRIG_NOTE:
              ...
              if ((row.volume == 0 || row.volume > 80) && voice.retrigx != 0) retrig(voice);
...
{j = _tmp_;
          ...
          if ((instr.volData.flags & ENVELOPE_ON) != 0) instr.volEnabled = 1;
          if ((instr.panData.flags & ENVELOPE_ON) != 0) instr.panEnabled = 1;
...
j = _tmp_;
        ...
        ipos = stream.position;
        if ((ipos : UInt) >= stream.length) break;
      }
