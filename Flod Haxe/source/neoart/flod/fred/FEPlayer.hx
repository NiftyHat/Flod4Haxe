...
final class FEPlayer extends AmigaPlayer
{
	var songs:Vector<FESong>;
	var samples:Vector<FESample>;
	var patterns:ByteArray;
	var song:FESong;
	var voices:Vector<FEVoice>;
	var complete:Int = 0;
	var sampFlag:Int = 0;
	...
	{
		var chan:AmigaChannel, i:Int, j:Int, len:Int, loop:Int, pos:Int, sample:FESample, value:Int, voice = voices[3];
		...
		while (voice != null)
		{
			...
			do
			{
				...
				if (voice.busy == 0)
				{
					...
					{
						chan.pointer = (sample.type != 0) ? voice.synth : sample.pointer + sample.loopPtr;
					...
					while (loop > 1)
					{
						...
						if (value < 0)
						{
							switch (value)
							{
								...
								case -127:
									value = (sample != null) ? sample.relative : 428;
						...
						else
						{
							...
							if (sample.type == 1)
							{
								if (sampFlag != 0 || ((sample.synchro & 2) != 0))
								{
			...
			if (voice.portaFlag != 0)
			{
				...
				{
					voice.period += Std.int((voice.portaCounter * voice.portaPeriod) / voice.portaSpeed);
			...
			if (sample.type == 1)
			{
				if (voice.pulseDelay != 0)
				{
					voice.pulseDelay--;
				}
				else
				{
					if (voice.pulseSpeed != 0)
					{
						voice.pulseSpeed--;
					}
					else
					{
						if (voice.pulseCounter != 0 || !((sample.synchro & 1) != 0))
						{
							voice.pulseSpeed = sample.pulseSpeed;

							if ((voice.pulseDir & 4) != 0)
							{
								while (1 != 0)
								{
			...
			{
				if (voice.blendDelay != 0)
				{
					voice.blendDelay--;
				}
				else
				{
					if (voice.blendCounter != 0 || !((sample.synchro & 4) != 0))
					{
						if (voice.blendDir != 0)
						{
							if (voice.blendPos != 1)
	...
	{
		var i:Int, len:Int, voice = voices[3];
		super.initialize();
	...
	{
		var basePtr = 0, dataPtr = 0, i:Int, j:Int, len:Int, pos:Int, ptr:Int, sample:FESample, size:Int, song:FESong, tracksLen:Int, value:Int;
		...
		while ((pos : UInt) > stream.position)
		{
			value = stream.readUnsignedInt();

			if (value != 0)
			{
				if (((value : UInt) < stream.position) || ((value : UInt) >= stream.length))
				{
					stream.position -= 4;
					break;
				}
			...
			{
				i = _tmp_;
				sample.arpeggio[i] = stream.readByte();
			};
	...
	final PERIODS:Vector<Int> = Vector.ofArray([
		8192, 7728, 7296, 6888, 6504, 6136, 5792, 5464, 5160, 4872, 4600, 4336, 4096, 3864, 3648, 3444, 3252, 3068, 2896, 2732, 2580, 2436, 2300, 2168, 2048,
		1932, 1824, 1722, 1626, 1534, 1448, 1366, 1290, 1218, 1150, 1084, 1024, 966, 912, 861, 813, 767, 724, 683, 645, 609, 575, 542, 512, 483, 456, 430, 406,
		383, 362, 341, 322, 304, 287, 271, 256, 241, 228, 215, 203, 191, 181, 170, 161, 152, 143, 135
	]);
}
