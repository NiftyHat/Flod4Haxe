              ...
              if (value < 0) {
                    ...
                    while (1 != 0) {
                      ...
                      if (value == 65535) {
                        ...
                        if (loopSong == 0) {
                          ...
                          if (complete == 0) amiga.complete = 1;
                        }
                      } else {
                ...
                if (sample.type == 1) {
                  if (sampFlag != 0 || ((sample.synchro & 2) != 0)) {
                    ...
                    while (i < len) {amiga.memory[i] = sample.pulseRateNeg;
++i;
};
                    len += (sample.length - sample.pulsePosL);
                    while (i < len) {amiga.memory[i] = sample.pulseRatePos;
++i;
};
                  }
                  ...
                  j = voice.synth;
                  len = i + 31;
                  while (i < len) {amiga.memory[j++] = amiga.memory[i];
++i;
};
...
sample.arpeggio[i] = stream.readByte();
        ...
        stream.position += 12;
        samples.push(sample);
        if (stream.bytesAvailable == 0) break;
      }
...
i = _tmp_;
          sample = samples[i];
          if (sample.pointer != 0) sample.pointer -= (basePtr + pos);
        }
      }
