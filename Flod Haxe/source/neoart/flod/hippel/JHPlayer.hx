...
final class JHPlayer extends AmigaPlayer
{
	var songs:Vector<JHSong>;
	...
	var frqseqs:Int = 0;
	var volseqs:Int = 0;
	var samplesData:Int = 0;
	var song:JHSong;
	var voices:Vector<JHVoice>;
	var coso:Int = 0;
	...
	{
		var chan:AmigaChannel, loop:Int, period:Int, pos1:Int, pos2:Int, sample:AmigaSample, value:Int, voice = voices[0];
		...
		while (voice != null)
		{
			...
			if (value >= 0)
				value += (voice.note + voice.trackTransp);
			value &= 127;
			...
			else
			{
				...
				{
					if (voice.vibrato >= 0 || !((voice.vibrato & 1) != 0))
					{
						if ((voice.vibrato & 32) != 0)
						{
			...
			if (variant > 3)
			{
				value = Std.int((voice.volFade * voice.volume) / 100);
			}
			else
			{
				value = voice.volume;
	...
	{
		var headers:Int, i:Int, id = 0, len:Int, pos:Int, sample:AmigaSample, song:JHSong, songsData:Int, tracks:Int, value = 0;

		base = periods = 0;
		coso = cast(stream.readMultiByte(4, CorePlayer.ENCODING) == "COSO");
		...
		if (coso != 0)
		{
			...
			switch (value)
			{
				case 22666 | 18842 | 30012 | 22466 | 3546:
					variant = 1;

				case 16948 | 18332 | 13698:
					variant = 2;

				case 18546 // Wings of Death
					| 13926 | 8760 | 17242 | 11394 | 14494 | 14392 | 13576 // Dragonflight
					| 6520:
					variant = 3;
			...
			len = Std.int(((samplesData - headers) / 10) - 1);
			lastSong = Std.int((headers - songsData) / 6);
		}
		else
		{
			while (stream.bytesAvailable > 12)
			{
				...
				switch (value)
				{
					...
					case 0x7002 // moveq #2,d0
						...
						value = stream.readUnsignedShort();
						if (value == 0x7600)
							value = stream.readUnsignedShort(); // moveq #0,d3
		...
		for (_tmp_ in 0...len)
		{
			...
			if (coso == 0)
				sample.name = stream.readMultiByte(18, CorePlayer.ENCODING);
	...
	final PERIODS:Vector<Int> = Vector.ofArray([
		...
		113, 113, 3424, 3232, 3048, 2880, 2712, 2560, 2416, 2280, 2152, 2032, 1920, 1812, 6848, 6464, 6096, 5760, 5424, 5120, 4832, 4560, 4304, 4064, 3840, 3624
	]);
}
