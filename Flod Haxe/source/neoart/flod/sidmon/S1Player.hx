  ...
  import neoart.flod.core.*;
   ...
   final class S1Player extends AmigaPlayer {
    ...
    override public function process() {
      var chan:AmigaChannel, dst:Int, i:Int, index:Int, memory= amiga.memory, row:SMRow, sample:S1Sample, src1:Int, src2:Int, step:AmigaStep, value:Int, voice= voices[0];
      ...
      while (voice != null) {
        ...
        if (tick == 0) {
          if (patternEnd != 0) {
            ...
            if (doReset != 0) voice.noteTimer = 0;
          }
          ...
          if (voice.noteTimer == 0) {
            ...
            if (row.sample == 0) {
              ...
              if (voice.waitCtr != 0) chan.enabled = voice.waitCtr = 0;
        ...
        if (sample.volume != 0) {chan.volume = sample.volume;}
...
sample.arpeggio[j] = stream.readUnsignedByte();
        ...
        if (sample.waveform > 15) {
          if ((totSamples > 15) && (sample.waveform > totSamples)) {
            ...
            if ((start : UInt) >= stream.length) continue;
            j = stream.position;
...
i = _tmp_;
    ...
    static inline final SIDMON_0FFA :ASAny= 0x0ffa;
    static inline final SIDMON_1170 :ASAny= 0x1170;
    static inline final SIDMON_11C6 :ASAny= 0x11c6;
    static inline final SIDMON_11DC :ASAny= 0x11dc;
    static inline final SIDMON_11E0 :ASAny= 0x11e0;
    static inline final SIDMON_125A :ASAny= 0x125a;
    static inline final SIDMON_1444 :ASAny= 0x1444;

    final EMBEDDED: Vector<Int> = Vector.ofArray([1166, 408, 908]);

    final PERIODS: Vector<Int> = Vector.ofArray([0,
