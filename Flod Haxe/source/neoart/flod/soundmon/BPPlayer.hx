  ...
  import neoart.flod.core.*;
   ...
   final class BPPlayer extends AmigaPlayer {
    ...
    override public function process() {
      var chan:AmigaChannel, data:Int, dst:Int, instr:Int, len:Int, memory= amiga.memory, note:Int, option:Int, row:AmigaRow, sample:BPSample, src:Int, step:BPStep, voice= voices[0];
      ...
      while (voice != null) {
        ...
        if (voice.lfoControl != 0) {
          if (--voice.lfoCtr == 0) {
            ...
            if (sample.lfoDepth != 0) data = Std.int(data / sample.lfoDepth) >> 0;
          ...
          if (--voice.egCtr == 0) {
            ...
            if (voice.egValue != data) {
              ...
              if (voice.egValue < data) {
                data -= voice.egValue;
                len = dst - data;
                while (dst > len) memory[--dst] = buffer[--src];
              } else {
                data = voice.egValue - data;
                len = dst + data;
                while (dst < len) memory[dst++] = ~buffer[src++] + 1
;              }
            }
...
dst = _tmp_;
          ...
          case 6:   //wave change
            if (--voice.fxCtr == 0) {
              ...
              src = voice.synthPtr + 64;
              len = voice.synthPtr + 32;
              for (_tmp_ in voice.synthPtr...len) {dst = _tmp_;
memory[dst] = memory[src++];
      ...
      if (--tick == 0) {
        ...
        while (voice != null) {
          chan = voice.channel;
          if (voice.enabled != 0) chan.enabled = voice.enabled = 0;
          if (voice.restart == 0) {
            voice = voice.next;
            continue;
          }
        ...
        while (voice != null) {
          ...
          if (sample.synth != 0) {
            ...
            if (voice.egControl != 0 || voice.fxControl != 0 || voice.modControl != 0) {
              ...
              dst = voice.index << 5;
              len = voice.synthPtr + 32;
              for (_tmp_ in voice.synthPtr...len) {src = _tmp_;
...
i = _tmp_;
      ...
      i = 0;while (++i < 16) {
        sample = samples[i];
        if (sample.synth != 0 || sample.length == 0) continue;
        sample.pointer = amiga.store(stream, sample.length);
        sample.loopPtr = sample.pointer + sample.loop;
      }
    }

    static inline final BPSOUNDMON_V1 :ASAny= 1;
    static inline final BPSOUNDMON_V2 :ASAny= 2;
    static inline final BPSOUNDMON_V3 :ASAny= 3;
