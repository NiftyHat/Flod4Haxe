...
{i = _tmp_;
            ...
            if ((voice.step & 0x0ff0) == 0x0ed0) {
              if (voice.funkSpeed != 0) updateFunk(voice);
              extended(voice);
              voice = voice.next;
              continue;
            }
          ...
          while (voice != null) {
            chan = voice.channel;
            if (voice.enabled != 0) chan.enabled = 1;
...
i = _tmp_;
        ...
        if (row.sample > 31 || samples[row.sample] == null) row.sample = 0;
...
i = _tmp_;
        ...
        for (_tmp_ in sample.pointer...size) {j = _tmp_;
amiga.memory[j] = 0;
};
      }
...
++i;
          ...
          case 4   //vibrato
            ...
            if (voice.effect == 6) {
              ...
              if (value != 0) voice.vibratoParam = (voice.vibratoParam & 0xf0) | value;
              value = voice.param & 0xf0;
              if (value != 0) voice.vibratoParam = (voice.vibratoParam & 0x0f) | value;
            }
          ...
          case 7:   //tremolo
            ...
            if (voice.param != 0) {
              value = voice.param & 0x0f;
              if (value != 0) voice.tremoloParam = (voice.tremoloParam & 0xf0) | value;
              value = voice.param & 0xf0;
              if (value != 0) voice.tremoloParam = (voice.tremoloParam & 0x0f) | value;
            }
    ...
    function moreEffects(voice:PTVoice) {
      ...
      switch (voice.effect) {
        case 9:   //sample offset
          if (voice.param != 0) voice.offset = voice.param;
        ...
        case 15:  //set speed
          ...
          if (voice.param < 32) {speed = voice.param;}
            else {amiga.samplesTick = Std.int(110250 / voice.param);}
    ...
    function extended(voice:PTVoice) {
      ...
      switch (effect) {
        ...
        case 6:   //pattern loop
          ...
          if (param != 0) {
            if (voice.loopCtr != 0) {voice.loopCtr--;}
        ...
        case 8:   //karplus strong
          ...
          i = voice.loopPtr;while (i < len)
            memory[i] = Std.int((memory[i] + memory[++i]) * 0.5);

          memory[++i] = Std.int((memory[i] + memory[0]) * 0.5);
          
        case 9:   //retrig note
          if (tick != 0 || param == 0 || voice.period == 0) return;
          if (tick % param != 0) return;
        ...
        case 13:  //note delay
          if (tick != param || voice.period == 0) return;
        ...
        case 14:  //pattern delay
          if (tick != 0 || patternDelay != 0) return;
          patternDelay = ++param;
          
        case 15:  //funk repeat or invert loop
          if (tick != 0) return;
          voice.funkSpeed = param;
          if (param != 0) updateFunk(voice);
    ...
    final FUNKREP: Vector<Int> = Vector.ofArray([
