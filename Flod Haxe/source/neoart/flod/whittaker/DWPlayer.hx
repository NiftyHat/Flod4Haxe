   ...
   final class DWPlayer extends AmigaPlayer {
        ...
        if (--voice.tick == 0) {
                ...
                switch (value) {
                  case -128:
                    ...
                    if (value != 0) {
                      ...
                      stream.position = base + ASCompat.toInt((stream : ASAny)[readMix]() );
                      voice.trackPos = readLen;
          ...
          if (voice.vibrato != 0) {
            if (voice.vibrato > 0) {
              ...
              if (voice.vibratoDelta == 0) voice.vibrato ^= 0x80000000;
            } else {
              voice.vibratoDelta += voice.vibratoSpeed;
              if (voice.vibratoDelta == voice.vibratoDepth) voice.vibrato ^= 0x80000000;
            }

            if (voice.vibratoDelta == 0) voice.vibrato ^= 1;
    ...
    override function initialize() {
      ...
      if (wave != null) {
        ...
        len = wavePos;
        while (i < len) {amiga.memory[i] = waveRateNeg;
++i;
};

        len += waveCenter;
        while (i < len) {amiga.memory[i] = waveRatePos;
++i;
};
      }
      ...
      while (voice != null) {
        ...
        voice.trackPos   = readLen;
        stream.position  = voice.trackPtr;
        voice.patternPos = base + ASCompat.toInt((stream : ASAny)[readMix]() );
    ...
    override function loader(stream:ByteArray) {
      ...
      while (1 != 0) {
        ...
        i = 0;while (i < channels) {
          value = base + ASCompat.toInt((stream : ASAny)[readMix]() );
...
++i;
        ...
        songs[total++] = song;
        if ((lower - stream.position) < (size : UInt)) break;
      }
...
songs[i].delay *= value;
          ...
          case 0x7e00                                                          //moveq #0,d7
            ...
            if (active != 0) {
              voices[0].next = null;
              i = total;while (i > 0) voices[i].next = voices[--i];
            } else {
              voices[total].next = null;
              i = 0;while (i < total) voices[i].next = voices[++i];
            }
          ...
          case 0x4ed2:                                                          //jmp a2
            ...
            if (frqseqs != 0) stream.position = stream.length;
